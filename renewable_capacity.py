...
filtered_renewable_capacity = installed_renewable_capacity[installed_renewable_capacity['Technology'] == 'Total renewable energy']
# print(filtered_renewable_generation)

# 1. Merge the necessary DataFrames based on the common column "CountryName"
merged_renewable_capacity = pd.merge(filtered_renewable_capacity, country_classifications, on="CountryName")
merged_population = pd.merge(population_data, country_classifications, on="CountryName")
merged_renewable_capacity.dropna(subset=["Region"], inplace=True)

# 2. Calculate the total electricity generation and enewable electricity generation per region
total_renewable_capacity_per_region = merged_renewable_capacity.groupby("Region")["2021"].sum().reset_index()
...
renewable_capacity_per_capita = pd.merge(total_renewable_capacity_per_region, total_population_per_region, on="Region")
renewable_capacity_per_capita["RenewableEnergyCapacityPerCapita"] = renewable_capacity_per_capita["2021_x"] / renewable_capacity_per_capita["2021_y"]
...
total_renewable_electricity_capacity = total_renewable_capacity_per_region["2021"].sum()
total_renewable_electricity_capacity_percapita = renewable_capacity_per_capita["RenewableEnergyCapacityPerCapita"].sum()
renewable_capacity_per_capita["PercentageOfTotalRenewableCapacityPerCapita"] = (renewable_capacity_per_capita["RenewableEnergyCapacityPerCapita"] / total_renewable_electricity_capacity_percapita) * 100

# 6. Create tables to display the results
result_table_renewable_capacity = renewable_capacity_per_capita[["Region", "2021_x", "RenewableEnergyCapacityPerCapita", "PercentageOfTotalRenewableCapacityPerCapita"]]
result_table_renewable_capacity.columns = ["Region", "TotalRenewableEnergyCapacity", "RenewableEnergyCapacityPerCapita", "PercentageOfTotalRenewableCapacityPerCapita"]

# Print or display the result table
print(result_table_renewable_capacity)